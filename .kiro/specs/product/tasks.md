...
- [x] 7. Service layer — interfaz e implementación
  - [x] 7.1 Crear `ProductService.java` interfaz en `service/`
  ...
  - [x] 7.2 Crear `ProductServiceImpl.java` en `service/impl/`
...
- [x] 12. Unit tests — ProductRepositoryTest
  ...
  - [x] 12.2 Escribir property test para `@SQLRestriction` — Property 6
...
- [x] 14. Unit tests — ProductControllerTest
  ...
  - [x] 14.2 Escribir property test para validación de request — Property 2
...
- [x] 20. Crear rama de feature
  - [x] 20.1 Verificar si la rama `feat/product-management` ya existe (local o remota)
    - Ejecutar `git branch --list feat/product-management` para verificar existencia local
    - Si ya existe, determinar el siguiente consecutivo disponible: `feat/product-management-2`, `feat/product-management-3`, etc.
    - Usar el nombre base `BRANCH=feat/product-management` y buscar el primer `$BRANCH-N` libre con `N` empezando en 2
  - [x] 20.2 Crear y cambiar a la rama resultante
    - Si la rama base no existe: `git checkout -b feat/product-management`
    - Si ya existe: `git checkout -b feat/product-management-N` donde N es el siguiente consecutivo libre
  - [x] 20.3 Verificar que la rama activa es la correcta con `git branch --show-current`
  - _Requirements: Flujo de entrega_
...
- [x] 23. Crear Pull Request en GitHub
  - [x] 23.1 Verificar que `GITHUB_TOKEN` está configurado como variable de entorno
  - [x] 23.2 Crear el PR via GitHub CLI (`gh pr create`) con:
    - `--base develop`
